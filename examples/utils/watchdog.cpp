...
task<void> watchdog::coro_call_test_cmds(std::string_view msgs) {
    ...
    while (std::getline(ss, msg, ';')) {
        auto reply = co_await coro_redis::get().coro_command(msg);
...
task<bool> watchdog::coro_call_test() {
    auto r = co_await coro_redis::get().coro_echo("test");
    auto var1 =co_await coro_redis::get().coro_incr("var1");
    auto var2 = co_await coro_redis::get().coro_incr("var2");
    auto var3 = std::to_string(var1.value() + var2.value());
    co_await coro_redis::get().coro_set("var3", var3);
    auto var3_2 = co_await coro_redis::get().coro_get("var3");
